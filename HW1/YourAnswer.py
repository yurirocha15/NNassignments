...
def naive_softmax_loss(Weights,X_data,Y_data,reg):
    ...
    # To avoid numerical instability when calculating the exponential (i.e. exponential of reaLLy large numbers)
    # we can subtract the output by its max value, which will make 0 the highest value. This assures that there will
    # be no instability without changing the log output (the constant on the numerator cancels out the one in the denominator)
...
def vectorized_softmax_loss(Weights, X_data, Y_data, reg):
    ...
    softmax_matrix[range(n), Y_data] -= 1
    dWeights = (X_data.T.dot(softmax_matrix)) / n
    dWeights += reg * Weights
